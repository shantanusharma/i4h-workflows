...
class StreamLiftDownSamplerApp(Application):
    def __init__(self, data=None):
        super().__init__()

        self.name = "Stream Lift Down Sampler App"
        if data is None:
    ...
    def compose(self):
        ...
        streamlift = StreamLiftDownSamplerOp(
            self,
            cuda_device_ordinal=0,
            name="streamlift_downsampler",
            allocator=UnboundedAllocator(self, name="streamlift_pool"),
        )

        visualizer = HolovizOp(self, name="holoviz", framebuffer_srgb=True, width=1920, height=1080)
        stats = StreamliftStatsOp(self, name="streamlift_stats", upsampling=False, interval_ms=1000)
...
if __name__ == "__main__":
    ...
    # convert_video_to_gxf_entities.py --width 3840 --height 2160 --channels 3 --framerate 30 --basename sample
    app = StreamLiftDownSamplerApp(data="/workspace/i4h-workflows/third_party/holohub/data/sample4k/")
    app.run()
